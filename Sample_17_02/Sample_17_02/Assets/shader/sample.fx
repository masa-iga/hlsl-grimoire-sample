...
void chs(inout RayPayload payload, in BuiltInTriangleIntersectionAttributes attribs)
{
    ...
    const uint polygonNo = PrimitiveIndex();

    const uint v0_id = g_indexBuffers[polygonNo * 3];
    const uint v1_id = g_indexBuffers[polygonNo * 3 + 1];
    const uint v2_id = g_indexBuffers[polygonNo * 3 + 2];

    // step-2 頂点番号から各頂点のUV座標を取得する
    const float2 uv0 = g_vertexBuffers[v0_id].uv;
    const float2 uv1 = g_vertexBuffers[v1_id].uv;
    const float2 uv2 = g_vertexBuffers[v2_id].uv;
    ...
    const float2 uv = barycentrics.x * uv0 + barycentrics.y * uv1 + barycentrics.z * uv2;
